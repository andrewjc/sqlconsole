...
{
    class ProviderORACLE:DatabaseProvider
    {
        ...
        public override FieldDescriptor[] getTableDescription(string table)
        {
            try
            {
                ...
                System.Data.Odbc.OdbcDataReader odr = this._dbObj.GetDatabaseConnector().getResult(String.Format("SELECT column_name \"Name\", concat(concat(concat(data_type,'('),data_length),')') \"Type\",nullable \"Null?\" FROM user_tab_columns WHERE table_name like '{0}'", table));
                while (odr.Read())
                {
                    ...
                    */
                     ...
                     */
                    fd.modifiers = new ABSTRACTFIELDMODIFIERS[0];
        ...
        public override AField typeStrToField(string typestr)
        {
            ...
            switch (sType.ToLower())
            {
                ...
                case "char":
                    ...
                    break;
                case "varchar":case "varchar2":case "nvarchar":case "nvarchar2":case "text":
