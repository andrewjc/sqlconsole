...
{
    class ProviderMSSQL2000:DatabaseProvider
    {
        ...
        private Boolean isPKCol(string sourcetable, string col)
        {
            ...
            while (indkey < 16 && this._dbObj.getNewConnection().GetDatabaseConnector().getSingleVal("select index_col('" + sourcetable + "',1," + indkey + ")", false) != null)
            {
                if ((string)this._dbObj.getNewConnection().GetDatabaseConnector().getSingleVal("select index_col('" + sourcetable + "',1," + indkey + ")", false) == col)
                {
                    foundOK = true;
                    break;
                }
